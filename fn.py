    ...
    for i, (start_p, end_p) in enumerate(l_pair):
        if start_p in part_line and end_p in part_line:
            ...
            cv2.line(
                ...
                end_xy,
                line_color[i],
                int(1 * (pts[start_p, 2] + pts[end_p, 2]) + 1),
            )
    return frame
...
def vis_frame_fast(frame, im_res, joint_format="coco"):
    ...
    for human in im_res:  # ['result']:
        ...
        kp_preds = torch.cat(
            (kp_preds, torch.unsqueeze((kp_preds[1, :] + kp_preds[2, :]) / 2, 0))
        )
        kp_scores = torch.cat(
            (kp_scores, torch.unsqueeze((kp_scores[1, :] + kp_scores[2, :]) / 2, 0))
        )
        # Draw keypoints
        ...
        for i, (start_p, end_p) in enumerate(l_pair):
            if start_p in part_line and end_p in part_line:
                ...
                cv2.line(
                    ...
                    end_xy,
                    line_color[i],
                    int(2 * (kp_scores[start_p] + kp_scores[end_p]) + 1),
                )
