...
def create_csv(folder):
    ...
    for fil in list_file:
        print(fil)
        number = f"0{fil}" if len(fil) == 1 else fil
        frames_count = 0
        for frame in os.listdir(os.path.join(folder, fil, f"fall-{number}-cam0-rgb")):
        ...
        df = df._append(pd.DataFrame(rows, columns=cols), ignore_index=True)
        # cap.release()
...
while True:
    ...
    if ret:
        ...
        frame = cv2.putText(
            frame,
            "Frame: {} Pose: {}".format(i + 1, cls_name),
            (10, 20),
