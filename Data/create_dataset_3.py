...
csv_pose_file = "Data/output/UR_falls-pose+score.csv"  # from create_dataset_2.py
save_path = "Data/output/UR_falls-set(labelXscrw).pkl"
# Params.
...
def seq_label_smoothing(labels, max_step=10):
    ...
    for i in range(labels.shape[0]):
        if remain_step > 0:
            if i >= start_change:
                ...
                labels[i][target_label] = (
                    max_val * (steps - remain_step) / steps
                    if max_val * (steps - remain_step) / steps
                    else min_val
                )
                remain_step -= 1
            continue
        ...
        diff_index = np.where(
            np.argmax(labels[i : i + max_step], axis=1) - np.argmax(labels[i]) != 0
        )[0]
        if len(diff_index) > 0:
...
for vid in vid_list:
    ...
    for fs in frames_set:
        ...
        # Add center point.
        xys = np.concatenate(
            (xys, np.expand_dims((xys[:, 1, :] + xys[:, 2, :]) / 2, 1)), axis=1
        )
        ...
        for i in range(xys.shape[0] - n_frames):
            feature_set = np.append(
                feature_set, xys[i : i + n_frames][None, ...], axis=0
            )
            labels_set = np.append(
                labels_set, lb[i : i + n_frames].mean(0)[None, ...], axis=0
            )
